...
import {testFatalErrorLabel} from './const';
...
export function setAxiosStub(
  ...
  for (const methodStub of methodStubs) {
    let stubResponse;
    if (methodStub.httpStatus === 200) {
      const response = {
        status: methodStub.httpStatus,
        data: methodStub.responseData,
      };
      stubResponse = new Promise(r => r(response));
    } else {
      const response = {
        response: {
          status: methodStub.httpStatus,
          statusText: testFatalErrorLabel,
        },
      };
      stubResponse = new Promise((_r, e) => e(response));
    }
